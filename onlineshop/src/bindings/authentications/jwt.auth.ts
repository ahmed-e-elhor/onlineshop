...
export class MyAuthMetadataProvider extends AuthMetadataProvider {
  constructor(@inject(CoreBindings.CONTROLLER_CLASS, {optional: true}) protected _controllerClass: Constructor<{}>, @inject(CoreBindings.CONTROLLER_METHOD_NAME, {optional: true}) protected _methodName: string) {
...
export const JWT_SECRET = '56khj934hjt@78g';
...
export class MyAuthAuthenticationStrategyProvider implements Provider<AuthenticationStrategy | undefined> {
  constructor(@inject(AuthenticationBindings.METADATA) private metadata: MyAuthenticationMetadata, @repository(UserRepository) private userRepository: UserRepository, @repository(RoleRepository) private roleRepository: RoleRepository) {}
      ...
      const jwtStrategy = new JwtStrategy(
        {
          ...
          jwtFromRequest: ExtractJwt.fromExtractors([ExtractJwt.fromAuthHeaderAsBearerToken(), ExtractJwt.fromUrlQueryParameter('access_token')]),
        },
        (payload: Credentials, done: (err: Error | null, user?: UserProfile | false, info?: Object) => void) => this.verifyToken(payload, done),
      );
  ...
  async verifyToken(payload: Credentials, done: (err: Error | null, user?: UserProfile | false, info?: Object) => void) {
    try {
      ...
      if (user.roleId) {
        ...
        if (userRoles && userRoles.permissions) userRoles.permissions = JSON.parse(userRoles.permissions);
      }
  ...
  async verifyRoles(userData: UserProfile) {
    ...
    if (type === SecuredType.HAS_ANY_ROLE) {
      if (!roles.length || !userData.roles.hasOwnProperty('permissions')) throw new HttpErrors.Unauthorized('Invalid authorization');
...
export class MyAuthActionProvider implements Provider<AuthenticateFn> {
  constructor(@inject.getter(MyAuthBindings.STRATEGY) readonly getStrategy: Getter<AuthenticationStrategy>, @inject.setter(AuthenticationBindings.CURRENT_USER) readonly setCurrentUser: Setter<UserProfile>, @inject.getter(AuthenticationBindings.METADATA) readonly getMetadata: Getter<MyAuthenticationMetadata>) {}
